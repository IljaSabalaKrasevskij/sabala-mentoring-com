...
/* ─────────────────────────────────────────────────────────────────────────
   SonarGrid — Punktfeld auf Canvas, das auf Ringe reagiert.
   Herkunft: 21st.dev, uebernommen am 12.9.2026. Zwei Aenderungen fuer dieses
   Projekt:

   1. KEIN erzwungener Fadenkreuz-Zeiger. Hier liegt das Feld als Hintergrund
      hinter Karten und Links, ein Fadenkreuz ueber Preisen und Knoepfen waere
      falsch. Wer es doch will, gibt `cursor-crosshair` per className mit.
   2. Die Farbe kommt aus `color`, nicht aus `text-primary`. Dieses Projekt hat
      kein Tailwind-Token `primary`, sondern eigene Marken-Variablen
      (`--gold`, `--gold-light`). Ohne `color` faellt es auf Messing zurueck.

   Das Feld ruht, wenn kein Ring lebt, haelt ausserhalb des Bildschirms und in
   versteckten Tabs an und zeigt bei Reduce Motion ein stilles Raster.
   ───────────────────────────────────────────────────────────────────────── */

export interface SonarGridProps extends React.ComponentProps<"div"> {
  /** Abstand der Punkte in CSS-Pixeln. */
  spacing?: number;
  /** Radius eines ruhenden Punktes in CSS-Pixeln. */
  dotRadius?: number;
  /** Deckkraft im Ruhezustand (0 bis 1). Punkte auf der Welle gehen auf 1. */
  baseOpacity?: number;
  /** Beliebige CSS-Farbe. Ohne Angabe Messing (--gold). */
  color?: string;
  /** Sekunden zwischen den Pulsen. 0 schaltet sie ab. */
  pingEvery?: number;
  /** Geschwindigkeit der Welle in CSS-Pixeln je Sekunde. */
  speed?: number;
  /** Dicke der Wellenfront in CSS-Pixeln. */
  ringWidth?: number;
  /** Wie stark ein Punkt am Wellenkamm waechst (0 = gar nicht, 2 = dreifach). */
  amplitude?: number;
  /** Puls dort ausloesen, wo getippt oder geklickt wird. */
  interactive?: boolean;
  /** Hoechstzahl gleichzeitiger Ringe. Aeltere fallen zuerst weg. */
  maxRings?: number;
  /** Mit einem bereits laufenden Ring starten, damit das erste Bild die Idee zeigt. */
  seedPing?: boolean;
  /** Wo Pulse entstehen duerfen, als Anteile von Breite und Hoehe: [x0, y0, x1, y1]. */
...
export function SonarGrid({
  ...
  React.useEffect(() => {
    ...
    const readColor = () => {
      stroke = getComputedStyle(canvas).color || MESSING;
    };
    ...
    const draw = (now: number) => {
      ...
      // Sekunden, bis ein Ring die Flaeche verlassen hat
      ...
      // Durchgang 1: alle ruhenden Punkte in einem Pfad, einmal gefuellt.
      const hot: number[] = [];
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (let j = 0; j < rows; j++) {
          ...
          for (const r of live) {
            ...
            const k = t * t * (3 - 2 * t) * r.fade; // weiche Flanke, verblasst mit dem Alter
      ...
      // Durchgang 2: nur Punkte auf der Wellenfront bekommen eigene Deckkraft und Groesse.
    ...
    const resize = () => {
      ...
      if (!seeded) {
        // Ein Ring laeuft schon, damit das erste Bild die Idee zeigt.
        seeded = true;
