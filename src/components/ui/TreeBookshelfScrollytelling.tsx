...
const START_FRAME = 1;
const READY_THRESHOLD = 24;     // Blog wird sichtbar, sobald die ersten 24 Frames geladen sind
const PRIORITY_BATCH_SIZE = 24; // Diese ersten Frames mit hoher Priorität parallel laden
...
function loadFrame(index: number): Promise<HTMLImageElement> {
  return new Promise((resolve) => {
    const img = new Image();
    // Async decoding hint — entlastet den Main-Thread während Initial-Render
    img.decoding = "async";
    img.src = getFramePath(index);
    img.onload = () => resolve(img);
    img.onerror = () => resolve(img); // Fallback: trotzdem auflösen, sortBy frame-num bleibt korrekt
  });
}
...
export default function TreeBookshelfScrollytelling() {
  ...
  // Frames werden in einem festen Array vor-allokiert und nach Index gefüllt
  // → keine teure Sortierung am Ende, kein Race-Condition zwischen Promises
  const [images, setImages] = useState<(HTMLImageElement | undefined)[]>(() =>
    Array.from({ length: FRAME_COUNT }, () => undefined)
  );
  ...
  // PRELOAD IMAGES — parallel statt sequentiell, mit progressivem Reveal
  useEffect(() => {
    let cancelled = false;
    const buffer: (HTMLImageElement | undefined)[] = Array.from(
      { length: FRAME_COUNT },
      () => undefined
    );
    let loaded = 0;
    ...
    const handleLoaded = (index: number, img: HTMLImageElement) => {
      if (cancelled) return;
      buffer[index - START_FRAME] = img;
      loaded++;
      setLoadedCount(loaded);

      // Sobald die Priority-Batch durch ist, Animation freigeben
      if (loaded === READY_THRESHOLD) {
        setImages([...buffer]);
        setIsReady(true);
      }
      // Bei Komplettladung nochmal aktualisieren (für volle Smoothness)
      if (loaded === FRAME_COUNT) {
        setImages([...buffer]);
      }
    };

    // 1. Priority-Batch: erste N Frames PARALLEL — Browser nutzt 6 Connections gleichzeitig
    const priorityFrames = Array.from(
      { length: Math.min(PRIORITY_BATCH_SIZE, FRAME_COUNT) },
      (_, i) => START_FRAME + i
    );
    Promise.all(
      priorityFrames.map((idx) => loadFrame(idx).then((img) => handleLoaded(idx, img)))
    ).then(() => {
      // 2. Rest-Batch: läuft im Hintergrund weiter, blockiert die UI nicht mehr
      const rest = Array.from(
        { length: FRAME_COUNT - PRIORITY_BATCH_SIZE },
        (_, i) => START_FRAME + PRIORITY_BATCH_SIZE + i
      );
      rest.forEach((idx) => {
        loadFrame(idx).then((img) => handleLoaded(idx, img));
      });
    });
