...
const teamMembers = [
  ...
  },
  {
    ...
    description: "Dein spezialisierter Kommunikationsassistent für englische Korrespondenz, schwierige Support-Tickets und professionelle Übersetzungen.",
    emotion: "Diese Support-Anfrage auf Englisch bereitet mir Bauchschmerzen — wie formuliere ich das professionell und verbindlich, ohne unhöflich zu klingen?",
    situations: ["Schwierige englische Support-Tickets beantworten", "Angebote ins Englische übersetzen", "Professionelle internationale Korrespondenz", "Verbindlich aber höflich formulieren"],
  ...
  },
  {
    ...
    description: "Der Erfindergeist deiner Custom GPT-Werkstatt. Hilft dir, eigene Assistenten zu bauen und dein KI-Team zu erweitern.",
    emotion: "Ich möchte mein KI-Team erweitern und für eine ganz spezielle Aufgabe meinen eigenen Custom GPT bauen — aber wie schreibe ich dafür die perfekten Instructions?",
    situations: ["Eigene Custom GPTs von Null konzipieren", "Instagram Content Creation Assistenten bauen", "Audio-Transkript Auswerter programmieren", "Perfekte Instruction-Architektur schreiben"],
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-10 relative z-50">
                      ...
                      <div className="relative group inline-block">
                        <button className={`flex items-center gap-2 border px-6 py-3 rounded-full text-sm font-bold tracking-widest transition-all cursor-crosshair shadow-[0_0_20px_rgba(0,0,0,0.5)] hover:scale-105 focus:scale-105 active:scale-95 ${member.textColor} border-${member.textColor.replace('text-', '')}/50 bg-${member.textColor.replace('text-', '')}/10 hover:bg-${member.textColor.replace('text-', '')}/20 focus:bg-${member.textColor.replace('text-', '')}/20`}>
                          <ScanFace className="w-5 h-5 animate-pulse" /> SCENARIO DATA
                        </button>
                        
                        {/* The Hover Popover */}
                        <div className="absolute left-0 bottom-full mb-6 w-[80vw] max-w-[450px] opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 pointer-events-none group-hover:pointer-events-auto group-focus-within:pointer-events-auto transition-all duration-300 translate-y-4 group-hover:translate-y-0 group-focus-within:translate-y-0 z-[100]">
                           {/* Connector Line */}
