...
export function DissolveParticleHero() {
  ...
  // Mobile-Detection: deaktiviert Canvas/Particles und Scroll-Driven-Animation
  useEffect(() => {
  ...
  useEffect(() => {
    // Auf Mobile keine Particle-Animation starten — spart CPU/Akku, vermeidet Ablenkung
    if (isMobile) return;
  ...
  return (
    <section
      ref={containerRef}
      // Mobile: normale Höhe, kein Sticky-Scroll-Effekt, kein 250vh-Container
      className="relative bg-warm-canvas rounded-b-[40px] z-10 shadow-[0_40px_60px_rgba(26,26,24,0.02)] h-auto md:h-[250vh]"
    >
      <div className="md:sticky md:top-0 md:h-[100dvh] pt-24 pb-16 md:pb-20 px-6 sm:px-12 md:px-24 flex items-center justify-center overflow-hidden">
        <div className="max-w-[1400px] w-full grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12 items-center relative z-10 h-full">

          {/* TEXT CONTENT — Mobile Order: 1 (Headline+Subtitle zuerst). Desktop: links neben Canvas */}
          <div className="lg:col-span-6 flex flex-col gap-5 z-20 xl:pr-8 order-1 md:order-1">
            {/* Pill — nur auf Desktop, oben im Text-Block */}
            ...
            <ScrollReveal delay={0.1}>
              <h1 className="font-instrument text-deep-charcoal leading-[1.1] tracking-[-0.02em] text-[clamp(2rem,7vw,3.5rem)] pb-2 flex flex-col gap-1">
            ...
            <ScrollReveal delay={0.2}>
              <p className="font-satoshi text-warm-steel text-[1rem] sm:text-[1.1rem] leading-[1.6] max-w-[520px]">
                Von der Kernbotschaft zur Premium-Webseite — mit professioneller Begleitung für mehr Conversion. Strategie, Branding und Technik aus einem Guss.
              </p>
            </ScrollReveal>
          </div>

          {/* VISUAL AREA — Mobile Order: 2 (nach Text). Desktop: rechts */}
          <div
            className="lg:col-span-6 relative w-full h-[55vh] sm:h-[60vh] lg:h-[85vh] flex items-end justify-center mt-2 md:mt-8 lg:mt-0 order-2 md:order-2"
          >
            {/* Mobile: einfaches Bild, ruhig, kein Scroll-Effekt, keine Animation */}
            <div className="md:hidden relative w-full h-full flex items-end justify-center">
            ...
            {/* Desktop: Particle-Canvas mit Scroll-Effekt */}
            <div
              className="hidden md:flex absolute inset-0 items-end justify-center"
              style={{
          ...
          {/* MOBILE-Pill — Order 3, NACH dem Bild. Desktop: hidden (Pill steht dort schon im Text-Block oben) */}
          <ScrollReveal className="md:hidden order-3 flex justify-center -mt-2">
            <div className="inline-block px-4 py-1.5 rounded-full border border-whisper-border bg-pure-surface text-[0.7rem] uppercase tracking-widest font-medium text-warm-steel">
              Der meditierende Business-Architekt
            </div>
          </ScrollReveal>
